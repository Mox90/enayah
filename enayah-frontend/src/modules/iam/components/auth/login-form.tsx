...
// enayah-frontend/src/modules/iam/components/auth/login-form.tsx
...
const Login = () => {
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      /* Clear stale data safety boundaries */
      await queryClient.cancelQueries()
      queryClient.clear()
    ...
    <div className='w-full'>
      <form onSubmit={handleLogin} className='space-y-6'>
        ...
        {error && (
          <div
            role='alert'
            className='flex items-start gap-3 rounded-xl border border-destructive/15 bg-destructive/5 px-4 py-3.5 text-sm text-destructive animate-in fade-in slide-in-from-top-1 duration-200'
          >
        ...
        <div className='space-y-4'>
          ...
          <div className='space-y-2 group'>
            ...
            <div className='relative group-focus-within:text-emerald-500 transition-colors'>
              <UserRound
                className={cn(
                  'pointer-events-none absolute top-1/2 size-4 -translate-y-1/2 text-muted-foreground/70 transition-colors duration-200',
                  isRtl ? 'right-4' : 'left-4',
                )}
              />
              ...
              <Input
                ...
                aria-invalid={Boolean(error)}
                className={cn(
                  //'h-12 rounded-xl bg-background/50 border-border/80 transition-all duration-200 focus-visible:ring-4 focus-visible:ring-emerald-500/10 focus-visible:border-emerald-500 focus-visible:bg-background',
                  'h-12 rounded-xl',
            ...
            <div className='relative group-focus-within:text-emerald-500 transition-colors'>
              ...
              <Input
                ...
                className={cn(
                  //'h-12 rounded-xl bg-background/50 border-border/80 transition-all duration-200 focus-visible:ring-4 focus-visible:ring-emerald-500/10 focus-visible:border-emerald-500 focus-visible:bg-background',
                  'h-12 rounded-xl',
                  isRtl ? 'pr-11 pl-12' : 'pl-11 pr-12', // Dynamic padding handles trailing toggles beautifully
                )}
              />
              ...
              <button
                ...
                onClick={() => setShowPassword((current) => !current)}
                className={cn(
                  'absolute top-1/2 flex size-8 -translate-y-1/2 items-center justify-center rounded-lg text-muted-foreground/60 transition-all hover:bg-muted hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500',
                  isRtl ? 'left-2' : 'right-2',
                )}
                aria-label={showPassword ? 'Hide password' : 'Show password'}
              >
        ...
        <Button
          className={cn(
            'h-12 w-full rounded-xl text-sm font-semibold tracking-wide shadow-md transition-all duration-200',
            'bg-emerald-600 hover:bg-emerald-500 text-white dark:bg-emerald-600 dark:hover:bg-emerald-500',
            'focus-visible:ring-4 focus-visible:ring-emerald-500/20 active:scale-[0.99]',
          )}
          type='submit'
          disabled={loading}
        >
